...
fn main() {
    ...
    let manifest_path = manifest_dir.join("ttrpg.toml");

    println!("cargo:rerun-if-changed=ttrpg.toml");

    let content = fs::read_to_string(&manifest_path).expect("cannot read ttrpg.toml");
    let manifest: toml::Value = content.parse().expect("cannot parse ttrpg.toml");
